...
int main() {
    ...
    S1.setUsuario(palabras);
    cout << "Ingresa la codigo: ";
    getline(cin,palabras);
    S1.setCodigo(palabras);
    ...
    try{
        ...
        do{
            ...
            if (i<MAX_ELEMENTS){
                ...
                switch(a) {
                    ...
                    getline(cin,palabras);
                    P1[P].setClave(palabras);
                    P1[P].getClave()+= ".txt";
                    archivo.open(P1[P].getClave().c_str(), ios::in);

                    if(archivo.fail()){
                        cerr << ">>> Nose encentra la clave del cliente " << P1[P].getClave() << ", la clave del cliente sera registrada al finalizar. \n";
                    }
                    ...
                    D1[D].getClave()+= ".txt";
                    archivo.open(D1[D].getClave().c_str(), ios::in);

                    if(archivo.fail()){
                        cerr << ">>> Nose encentra la clave del cliente " << D1[D].getClave() << ", la clave del cliente sera registrada al finalizar. \n";
                    }
        ...
        for(int i=0;i<MAX_ELEMENTS; i++){
            ...
            if (  P1[i].getCantidadPrestada() > 0 && P1[i].getCantidadDeInteres() > 0 && P1[i].getCantidadDeSemana() > 0){
                archivo.open(P1[i].getClave().c_str(), ios::app);
                archivo << "\n \n";
                archivo << "Prestamos----------------------------------------------------\n";
        ...
        for(int i=0;i<MAX_ELEMENTS; i++){
            ...
            if(D1[i].getCantidaddevuelta() > 0 && D1[i].getCantidadprestada() > 0 && D1[i].getSemanapagada() > 0 
                ...
                archivo.open(D1[i].getClave().c_str(), ios::app);
                archivo << "\n \n";
    ...
    } catch (string) { cout << "Tu usuario o clave no esta registrada";}

    cout << "\n \n";
    cout << "------------------------------------------------------------------------------------------\n";
	cout << "Creador: Jesus A. Mex Damian 23/03/2023.";
    return 0;
}
